...
import { ApiClient, tryDataRequest, createMnemonics, createWallet, getTransactions, getWalletInfos, lockWallet, unlockWallet, sendVTT } from '@/api'
...
export default new Vuex.Store({
  ...
  actions: {
    ...
    unlockWallet: async function (context, { walletId, password }) {
      const unlockWalletRequest = await unlockWallet(apiClient, { id: walletId, password })
      if (unlockWalletRequest.result) {
        context.commit('unlockWallet', context.store.wallet.id)
      } else {
